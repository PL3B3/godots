...
#	print("input size: ", client_inputs.size())
	if client_character != null and !client_inputs.is_empty():
		while client_inputs.size() > 2:
			print("Too many queued inputs, popping")
...
func add_random_movement(tick):
	...
	if tick % 60 == 0:
		var random_offset = 0.25 * Vector3.FORWARD.rotated(Vector3.UP, rng.randf_range(0, 2 * PI))
		client_character.position += random_offset

...
func _process(delta):
	#print(delta, " ", Engine.get_physics_interpolation_fraction())
...
func player_move(delta):
	...
	var floor_contact = FloorCheck.new(get_world_3d().direct_space_state, COLLISION_RADIUS, [self]).find_floor(global_position, floor_check_distance) # find_floor(global_position, floor_check_distance)
...
#const MIN_FLOOR_CONTACT_DEPTH:float = 0.000001

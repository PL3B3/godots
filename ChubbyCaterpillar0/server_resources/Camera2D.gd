...
func _physics_process(delta):
	...
	if ("velocity" in parent): # if parent is player
		...
		transform[2] += catch_up_rate * (target - transform[2])
	# Interpolator for smooth movement
	Interpolator.interpolate_property(self, "transform", get_transform(), transform, delta, Tween.TRANS_LINEAR, Tween.EASE_IN_OUT)

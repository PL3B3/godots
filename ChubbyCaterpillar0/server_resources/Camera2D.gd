...
var scouting_distance_x := 0.4 # how far ahead camera looks in direction of velocity_x
...
func _physics_process(delta):
	...
	if ("velocity" in parent): # if parent is player
		var target = Vector2(scouting_distance_x * parent.velocity.x, scouting_distance_y * parent.gravity2)
